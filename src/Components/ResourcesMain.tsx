                        ...
                        {allSubjects.length > 0 ?
                            allSubjects.map((i) => (
                                <li key={i._id} className="p-4">
                                    <Resource expandedSubject={expandedSubject} setExpandedSubject={setExpandedSubject} setExpandedEntryId={setExpandedEntryId} setExpandedSubId={setExpandedSubId} id={i._id} isAdmin={isAdmin} subject={i.subj} expandedEntryId={expandedEntryId} expandedSubId={expandedSubId} />
                                </li>
                            )) : expandedSubjectLoading ? <Loading /> : !expandedSubjectLoading && language !== "" ? <p className="flex flex-wrap text-zinc-600 justify-center">For this Language Resources not Available,<span className="text-blue-800 underline cursor-pointer" onClick={() => setSelectLangDialog(true)}> Select Language </span></p> : <p className="flex flex-wrap text-zinc-600 justify-center">First, Please Select Language, <span className="text-blue-800 underline cursor-pointer" onClick={() => setSelectLangDialog(true)}> Select Language </span> </p>}
                    </ul>
                </div>
            </section>
